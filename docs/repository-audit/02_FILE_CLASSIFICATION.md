...
**No file may be deleted, moved, or renamed without explicit approval.** All `DELETE CANDIDATE` rows are unapproved proposals. "Evidence" cites imports, tests, tooling, or git state verified during the audit.
...
| `airspacesim/routes/route_manager.py` | DELETE CANDIDATE (deprecation cycle) | Self-described compatibility shim re-exporting `manager.RouteManager`; kept alive by `tests/test_structure_cleanup.py`. Published API — needs deprecation, not silent removal. | Deprecate in 0.2.x, remove at the next breaking release. | Low |
...
| `airspacesim/static/`, `airspacesim/templates/map.html` | KEEP (legacy surface), MOVE later | Legacy Leaflet UI shipped in the wheel; exercised by `test_browser_console_clean.py`, `test_phase1_clean_run.py`. Long-term it does not belong in an open-source *engine* package. | Decide at the 1.0 boundary: extract to a separate compat package or drop. | Medium |
...
| `airspacesim/hello.py` + `say_hello` export | DELETE CANDIDATE | Tutorial artefact exported from `__init__`; referenced by `tests/test_integration.py` only. | Remove export + test at next minor release. | Low |
| `airspacesim/api/__init__.py`, `web/__init__.py`, `config/__init__.py`, `tests/__init__.py` | DELETE CANDIDATE | Empty/near-empty placeholder subpackages inside the engine; `config` only re-exports `settings`; the real API/web live in `apps/`. Nothing imports `airspacesim.api`/`airspacesim.web` (grep). | Remove after confirming no external users; `config` needs a deprecation shim. | Low |
| `airspacesim/data/*.json` (gao_airspace, airspace_data, airspace_config, map_config.v1, scenario_airspace.v1, scenario.v0.1, ui_runtime.v1, render_profile.v1) | REFACTOR (content replacement) | Package seed data is Gao-derived: `GAO_VOR`, real-looking fixes (ETRUL, PILTI, OPUGO…), real airway ids (UA612, UG859, UR971…). Brief: replace public Gao data with a coherent fictional environment. | Replace contents with the fictional environment; keep file names/contracts. `gao_airspace.json` → DELETE CANDIDATE (legacy fallback only). | Medium |
| `airspacesim/data/{aircraft_data,aircraft_state.v1,trajectory.v0.1,inbox_events.v1,aircraft_ingest,new_aircraft}.json` | KEEP (seed/runtime templates) | Empty-ish runtime output/ingest templates copied by `init`. `new_aircraft.json` is the legacy name (fallback in settings) — DELETE CANDIDATE with `aircraft_ingest.json` kept. | Keep; drop legacy `new_aircraft.json` at a breaking release. | Low |
...
| `airspaces/gao_demo/**` | REFACTOR (replace data) | Renamed "Sahel Control" but retains `gao_demo` id, `GAO_VOR`, real fixes/airways, "Gao Sector Traffic" scenario title. Referenced by `scripts/seed_hosted_demo.py`, `scripts/start_hosted_dev.py`, `apps/web/src/lib/simulateScenarios.ts`, `docs/user/*`. | Replace with a fully fictional pack (new id/name/fixes/routes); update all references in one change. | Medium |
...
| `dashboard.html` | INVESTIGATE | Untracked standalone design mock ("Simulation Console") with external Google-Fonts links; nothing references it. Likely a UI design spike for the run workspace (`docs/frontend/run_workspace_ux_spec.md` may relate). | Ask owner: archive under `docs/frontend/` as a design artefact, or delete. | Low |
...
| `lessons.md` | MOVE | Traffic Relationships build prompt (input spec, not repo doc); untracked, root-level. | Move into the brief folder or `docs/improvements/`. | Low |
