...
Create and maintain separate documentation for:
...
```text
09_DOCUMENTATION_AND_DEVELOPER_EXPERIENCE.md
```

At minimum, the repository must document:

- how to install and run the project;
- required tools and supported versions;
- exact commands and working directories;
- which services require separate terminals;
- frontend and backend browser URLs;
- local PostgreSQL setup;
- migrations;
- test commands;
- manual browser test flows;
- local test account creation or development-only credentials;
- authentication behaviour;
- environment variables;
- deployment;
- troubleshooting;
- lesson, scenario, environment, and translation authoring.

The root README should remain concise and link to detailed documentation.

The project must also include general user documentation that explains Learn, Practice, Simulate, guest access, accounts, language switching, debriefs, and the non-operational-use limitation in plain language.

Update documentation in the same change whenever commands, ports, environment variables, authentication, database, deployment, schemas, or user flows change.

## Repository hygiene

Create and maintain a suitable `.gitignore`.

Ignore secrets, dependency directories, virtual environments, build output, caches, logs, local databases, browser test artefacts, and machine-specific files.
...
Never commit production credentials or personal test credentials.
